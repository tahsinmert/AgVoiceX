...
export default function ReservationsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return reservations.filter((item) => {
      ...
      const haystack = [item.customer_name, item.phone, item.email, item.notes, item.status, details.type]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  const metrics = useMemo(() => {
    ...
    const todayItems = active.filter((item) => item.reservation_date === today);
    const confirmed = reservations.filter((item) => item.status === "confirmed");
    return {
      total: reservations.length,
      today: todayItems.length,
      guests: todayItems.reduce((sum, item) => sum + item.people, 0),
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Track bookings created by voice agents and staff.</p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <Metric icon={CalendarCheck2} label="Total records" value={metrics.total} />
        <Metric icon={Clock3} label="Today" value={metrics.today} />
        <Metric icon={Users} label="Guests today" value={metrics.guests} />
        <Metric icon={Check} label="Confirmed" value={metrics.confirmed} />
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <Select value={form.type} onChange={(event) => setForm({ ...form, type: event.target.value })}>
              {reservationTypes.map((item) => <option key={item} value={item}>{typeLabel(item)}</option>)}
            </Select>
          ...
          <CardHeader>
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <Badge className="bg-muted text-muted-foreground">{filtered.length} shown</Badge>
              </div>
              <div className="grid gap-2 md:grid-cols-[1fr_150px_150px_150px]">
                ...
                <Select value={status} onChange={(event) => setStatus(event.target.value)}>
                  {statuses.map((item) => <option key={item} value={item}>{typeLabel(item)}</option>)}
                </Select>
              </div>
            </div>
          </CardHeader>
...
function Metric({ icon: Icon, label, value }: { icon: typeof CalendarCheck2; label: string; value: number }) {
  return (
...
function ReservationRow({ item, onStatus }: { item: Reservation; onStatus: (id: number, status: string) => Promise<void> }) {
  const details = parseNotes(item.notes);
  const title = item.customer_name ?? `Reservation #${item.id}`;
  const primaryDetail = details.type === "hotel" && details.roomType
    ? `${details.roomType} room`
    : details.service || details.location || typeLabel(details.type);

  return (
    <div className="grid gap-3 rounded-md border bg-background p-3 text-sm lg:grid-cols-[minmax(0,1fr)_220px]">
      <div className="min-w-0 space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <p className="font-medium">{title}</p>
          <Badge className={statusClass(item.status)}>{typeLabel(item.status)}</Badge>
          <Badge className="bg-muted text-muted-foreground">{typeLabel(details.type)}</Badge>
        </div>
        <div className="grid gap-2 text-muted-foreground md:grid-cols-3">
          <span className="inline-flex items-center gap-1.5"><CalendarClock className="h-3.5 w-3.5" />{item.reservation_date} {item.reservation_time.slice(0, 5)}</span>
          <span className="inline-flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />{item.people} guests</span>
          {item.phone ? <span className="inline-flex items-center gap-1.5"><Phone className="h-3.5 w-3.5" />{item.phone}</span> : null}
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          {details.nights ? <Badge className="bg-muted text-muted-foreground">{details.nights} nights</Badge> : null}
          {details.checkoutDate ? <Badge className="bg-muted text-muted-foreground">Checkout {details.checkoutDate}</Badge> : null}
        </div>
        {details.freeText ? <p className="line-clamp-2 text-muted-foreground">{details.freeText}</p> : null}
      </div>
